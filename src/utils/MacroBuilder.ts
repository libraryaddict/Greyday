...
const substringCombatItems: Item[] =
  `spider web, really sticky spider web, dictionary, NG, Cloaca-Cola, yo-yo, top, ball, kite, yo, red potion, blue potion, adder, red button, pile of sand, mushroom, deluxe mushroom`
    .split(",")
    .map((s) => toItem(s));
const substringCombatSkills: Skill[] =
...
export class Macro {
   ...
   */
  save(): void {
    setProperty(Macro.SAVED_MACRO_PROPERTY, this.toString());
  }
  ...
  static load<T extends Macro>(this: Constructor<T>): T {
    return new this().step(
      ...getProperty(Macro.SAVED_MACRO_PROPERTY).split(";")
    );
  }
